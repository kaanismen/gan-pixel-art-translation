...
def soft_histogram(x, num_bins=16, min_val=0.0, max_val=1.0, sigma=0.01):
    """
    Compute a differentiable “soft” histogram over pixel values.

    Args:
        x (tensor): shape (B, C, H, W), values in [min_val, max_val]
        num_bins (int): number of bins in the histogram
        min_val (float): lower bound of pixel range
        max_val (float): upper bound of pixel range
        sigma (float): width of the Gaussian kernel for soft‐assignment

    Returns:
        hist (tensor): shape (B, C, num_bins), normalized per‐channel histogram
    """
    B, C, H, W = x.shape
    device = x.device
    # Create bin centers
    bin_centers = torch.linspace(min_val, max_val, steps=num_bins, device=device)  # (num_bins,)
    # Flatten spatial dims and add a bin dimension
    x_flat = x.view(B, C, H * W, 1)  # (B, C, N, 1)
    # Compute Gaussian weights for each bin center
    diff = x_flat - bin_centers  # (B, C, N, num_bins)
    weights = torch.exp(-0.5 * (diff / sigma) ** 2)
    # Sum over pixels to get raw histogram
    hist = weights.sum(dim=2)  # (B, C, num_bins)
    # Normalize so each per‐channel histogram sums to 1
...
def palette_loss(fake_img, real_img, num_bins=16, sigma=0.01):
    """
    Palette Loss between two batches of images.

    Args:
        fake_img (tensor): generated images, shape (B, C, H, W), in [0,1]
        real_img (tensor): target images,   shape (B, C, H, W), in [0,1]
        num_bins (int): number of histogram bins per channel
        sigma (float): Gaussian kernel width for soft binning

    Returns:
        loss (tensor): scalar L1 loss between palette histograms
    """
    # Compute soft histograms
    hist_fake = soft_histogram(fake_img, num_bins=num_bins, sigma=sigma)
    hist_real = soft_histogram(real_img, num_bins=num_bins, sigma=sigma)
    # L1 distance between the two histograms
    return F.l1_loss(hist_fake, hist_real)
...
def main(args):
   ...
   # )
    ...
    vae_enc = accelerator.prepare(vae_enc)
    vae_dec = accelerator.prepare(vae_dec)
    net_disc_a = accelerator.prepare(net_disc_a)
    net_disc_b = accelerator.prepare(net_disc_b)
    net_lpips = accelerator.prepare(net_lpips)

    optimizer_gen = accelerator.prepare(optimizer_gen)
    optimizer_disc = accelerator.prepare(optimizer_disc)
    train_dataloader = accelerator.prepare(train_dataloader)
    lr_scheduler_gen = accelerator.prepare(lr_scheduler_gen)
    lr_scheduler_disc = accelerator.prepare(lr_scheduler_disc)
    if accelerator.is_main_process:
    ...
    for epoch in range(first_epoch, args.max_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(*l_acc):
                ...
                Palette Loss
                """
                loss_palette_raw = torch.tensor(0.0, device=img_a.device)
                if args.palette_loss == "active":
                    # these two calls refer to your imported palette_loss function
                    loss_palette_raw = (
                        palette_loss(cyc_fake_b, img_b,
                                     num_bins=args.num_bins,
                                     sigma=args.sigma)
                      + palette_loss(cyc_fake_a, img_a,
                                     num_bins=args.num_bins,
                                     sigma=args.sigma)
                    )
                
                # 2) weight it
                weighted_palette_loss = args.lambda_palette * loss_palette_raw
                
                # 3) backward
                accelerator.backward(weighted_palette_loss, retain_graph=False)
                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(params_gen, args.max_grad_norm)
                optimizer_gen.step()
                lr_scheduler_gen.step()
