...
import torch.nn.functional as F1 #f.l1 loss içinde kullandık aynısı train_cyclegan_turbo.py'da da var clash olmaması için F1 çünkü
#training utilste var
from my_utils.training_utils import quantize_tensor

def main(args,use_retrain=None):
    ...
    #buraya image_edge=args.image_edge eklendi
    dataset_train = UnpairedDataset(dataset_folder=args.dataset_folder, image_prep=args.train_img_prep, image_edge=args.image_edge,split="train", tokenizer=tokenizer)
    ...
    #burayı ben ekledim type allias düzgün olsundiye
    fixed_caption_src = dataset_train.caption_a
    fixed_caption_tgt = dataset_train.caption_b
    l_images_src_test = []
    ...
    first_epoch = 0
    global_step = 0
    palette_every=args.palette_loss_every # out of memory almamak için
    ...
    for epoch in range(first_epoch, args.max_train_epochs):
        for step, batch in enumerate(train_dataloader):
            use_retrain = (args.palette_loss=="active")  #bunu ekledik  bu hali hazırda var ve pixel_art çalışırken çalışıyor
            ...
            with accelerator.accumulate(*l_acc):
                ...
                # accelerator.backward(loss_cycle_a + loss_cycle_b, retain_graph=False) bunu true'ya çevirdik çünkü palette
                #loss'ta geri gidip tekrar hesaplamamız gerekiyor
                #pixel_loss var mı yok mu datası use_retain içinde kalıyor bu hali hazırda mainde olan bir şeydi mainden kalktı
                # eğer true'ya çevirirsek out of memory alıyoruz
                """
                ...
                # palette loss on the cycle outputs _before_ we free the graph
                loss_palette = torch.tensor(0.0, device=img_a.device)
                if args.palette_loss == "active":
  # quantize the _cycle_ images, not the GAN fake
                   q_b = quantize_tensor(cyc_fake_b)
                   q_a = quantize_tensor(cyc_fake_a)
                   loss_palette = F1.l1_loss(cyc_fake_b, q_b) + F1.l1_loss(cyc_fake_a, q_a)

 # backprop cycle + palette
                cycle_total = (loss_cycle_a + loss_cycle_b) + args.lambda_palette * loss_palette
                optimizer_gen.zero_grad()
                accelerator.backward(cycle_total, retain_graph=False)

                if accelerator.sync_gradients:
                   accelerator.clip_grad_norm_(params_gen, args.max_grad_norm)
                optimizer_gen.step(); lr_scheduler_gen.step()
                ...
                """
                #accelerator.backward(loss_gan_a + loss_gan_b, retain_graph=False) bunu true'ya çevirdik çünkü palette
                #loss'ta geri gidip tekrar hesaplamamız gerekiyor
                #pixel_loss var mı yok mu datası use_retain içinde kalıyor bu hali hazırda mainde olan bir şeydi mainden kalktı
                #eğer true'ya çevirirsek out of memory alıyoruz
                ...
                #bu aşağıdaki loss_g_idt yani identity loss'u koruyacağız fakat buna eğer palette loss ekliyse
                #değişikliğe gitmemiz gerekecek
                optimizer_gen.zero_grad()
                accelerator.backward(loss_idt_a + loss_idt_b, retain_graph=False)
                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(params_gen, args.max_grad_norm)
                optimizer_gen.step();
                lr_scheduler_gen.step()
                #burada o değişikliğe gidiyoruz
                # palette loss'un active olma durumunda


                """
                    # eğer true'ya çevirirsek out of memory alıyoruz
                    accelerator.backward(loss_g_idt, retain_graph=False)
                    # bunuda false'tan aynı sebeple use retain'e çevirdik
                """
