    ...
    for epoch in range(first_epoch, args.max_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(*l_acc):
                ...
                total_gen_lost = ((loss_cycle_a + loss_cycle_b) + (loss_gan_a + loss_gan_b) + (loss_idt_a + loss_idt_b))
                if args.palette_loss== 'active':
                    quant_b = quantize_tensor(fake_b) # quant quantize fonksiyonundan gelmekte
                    quant_a = quantize_tensor(fake_a)
                    loss_palette= F1.l1_loss(fake_b, quant_b)+ F1.l1_loss(fake_a,quant_a)#L1 loss kullanmaktayız
                    total_gen_lost = total_gen_lost+ args.lambda_palette*loss_palette

                accelerator.backward(total_gen_lost)
                if accelerator.sync_gradients:
                     accelerator.clip_grad_norm_(params_gen, args.max_grad_norm)
                optimizer_gen.step()
                lr_scheduler_gen.step()
